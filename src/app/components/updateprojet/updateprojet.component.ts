...
//onUpdateProjet(): void {
  ...
  // if (this.projetFormGroup?.invalid) {
    ...
    // if (this.projetFormGroup?.controls['idemploye'].value === '') {
    //  this.projetFormGroup?.controls['idemploye'].setValue(this.projet?.cout);
  // }
...
//  }
  ...
  onUpdateProjet(): void {
    ...
    let projetupdate:Projet=this.projetFormGroup?.value;
    if(this.projet) {
      projetupdate.employe = this.projet.employe;
      this.projetService.updateProjet(projetupdate).subscribe({next: data => alert('Projet mis à jour'), error : err => alert(err.headers.get("error"))
      })
    }
  }
